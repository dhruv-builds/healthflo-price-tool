...
export function PricingToolbar({ currency, setCurrency, fxState, onManualRate, onExport, isPresentationMode, onTogglePresentationMode }: PricingToolbarProps) {
  const [editingRate, setEditingRate] = useState(false);
  const [rateInput, setRateInput] = useState(fxState.rate.toString());
  const { role } = useAuth();

  return (
    <div className="flex items-center justify-between border-b bg-card/80 px-6 py-2">
